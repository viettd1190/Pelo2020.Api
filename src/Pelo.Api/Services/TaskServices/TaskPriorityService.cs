                                    ...
                                    ITaskPriorityService
    {
        ...
        public async Task<TResponse<PageResult<GetTaskPriorityPagingResponse>>> GetPaging(int userId, GetTaskPriorityPagingRequest request)
        {
            try
            {
                ...
                if (canGetAll.IsSuccess)
                {
                    var result = await ReadOnlyRepository.QueryMultipleLFAsync<GetTaskPriorityPagingResponse, int>(string.Format(SqlQuery.TASK_PRIORITY_GET_BY_PAGING,request.ColumnOrder,request.SortDir),
                                                                                                              new
                                                                                                              {
                                                                                                                  Name = $"%{request.Name}%",
